...
4. **Решить вопрос с историей:** 
   - Если политика требует удаления из истории, используем `git-filter-repo` (force-push и оповестить команду).  
...
- **Triage-сообщения (ASOC):**  
  - TP/Critical:  
    > „Обнаружен секрет `<type>` в `<path>`. Удалить из HEAD + немедленно ротировать. Allowlist запрещён до подтверждения ротации.” 
  - TP/High:  
    > „Удалить из кода, ротировать по регламенту, затем повторный скан.” 
  - FP:  
    > „Mock/пример. Закрыть как FP, добавить в allowlist.”
